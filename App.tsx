...
const App: React.FC = () => {
  ...
  const handleLoadingComplete = () => {
    ...
    if (!hasPlayedAudio.current) {
      ...
      audio.play().catch((err) => {
        console.debug(
          "Audio playback prevented by browser policy. Interaction required.",
          err,
        );
      });
  ...
  const isAdminArea =
    currentView === "admin_login" || currentView === "admin_panel";
  ...
  const renderContent = () => {
    ...
    return (
      <>
        <nav
          id="nav-header"
          className={`fixed top-0 left-0 right-0 z-[1000] px-4 md:px-8 py-6 md:py-10 flex flex-col md:flex-row gap-6 md:justify-between md:items-center backdrop-blur-xl border-b transition-colors duration-700 ${isDarkMode ? "bg-black/90 border-white/5" : "bg-white/95 border-black/5"}`}
        >
          <div className="flex justify-between items-center w-full md:w-auto">
            <div
              className="flex items-center gap-5 clickable group"
              onClick={() => navigateTo("home")}
            >
              <div className="autography text-3xl md:text-5xl transition-transform group-hover:translate-x-1 text-current whitespace-nowrap text-white">
                Resource Forensic
              </div>
            </div>
          </div>
        ...
        <div
          id="theme-switcher"
          className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[2000] flex flex-col items-end gap-2 scale-90 md:scale-100"
        >
          <span className="text-[7px] tracking-[0.3em] uppercase opacity-20 font-black">
            SCANNER_MODALITY
          </span>
          <div
            className={`flex items-center gap-4 border p-1 rounded-sm transition-all duration-700 ${isDarkMode ? "border-white/10 bg-black/40" : "border-black/10 bg-white/40"}`}
          >
            <button
            ...
            <div
              className={`w-[1px] h-4 ${isDarkMode ? "bg-white/10" : "bg-black/10"}`}
            />
            <button
...
const HomeView: React.FC<{
  onCategorySelect: (cat: ResourceCategory) => void;
  onSearchTrigger: () => void;
  isDarkMode: boolean;
  resources: Resource[];
}> = ({ onCategorySelect, onSearchTrigger, isDarkMode, resources }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLogs((prev) =>
        [logPool[Math.floor(Math.random() * logPool.length)], ...prev].slice(
          0,
          10,
        ),
      );
  ...
  return (
    <div className="px-4 md:px-8 animate-in fade-in duration-1000 flex flex-col items-center">
      <div id="hero-area" className="mb-24 md:mb-48 text-center px-4">
        <h1 className="hero-title select-none uppercase pointer-events-none mb-2 md:mb-6 leading-[0.7] text-[18vw] md:text-[15vw]">
          RESOURCE
        </h1>
        <h1 className="hero-title select-none uppercase pointer-events-none leading-[0.7] text-[18vw] md:text-[15vw]">
          FORENSIC
        </h1>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 w-full max-w-7xl mb-32 md:mb-64">
        ...
        >
          <div className="flex items-center gap-6 md:gap-10">
            <svg
              className={`w-5 h-5 md:w-6 md:h-6 opacity-30 group-hover:opacity-100 transition-opacity`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
            ...
            <span className="text-[10px] md:text-[12px] tracking-[0.5em] md:tracking-[0.8em] uppercase font-black opacity-30 group-hover:opacity-100 transition-opacity">
              SEARCH THE ARCHIVES...
            </span>
          </div>

          <div className="flex items-center gap-6">
            <span className="text-[8px] tracking-[0.3em] uppercase opacity-10 font-black hidden md:block">
              PROTOCOL
            </span>
            <div
              className={`hidden md:block w-[1px] h-8 ${isDarkMode ? "bg-white/10" : "bg-black/10"}`}
            />
            <div
              className={`border rounded-sm px-3 py-1.5 transition-all ${isDarkMode ? "border-white/5 bg-white/[0.02]" : "border-black/5 bg-black/[0.02]"}`}
            >
              <span className="text-[8px] md:text-[10px] tracking-widest font-black opacity-40">
                CTRL + K
              </span>
            </div>
          </div>
        ...
        <div
          id="log-feed"
          className={`w-full md:w-80 h-32 md:h-24 border border-dashed p-4 flex flex-col font-mono overflow-hidden transition-colors ${isDarkMode ? "border-white/5 bg-white/[0.01]" : "border-black/5 bg-black/[0.01]"}`}
        >
          ...
          <div className="space-y-1">
            {logs.slice(0, 3).map((log, i) => (
              <div
                key={i}
                className={`text-[6px] tracking-tighter opacity-0 animate-in fade-in slide-in-from-left-2 duration-700 ${i === 0 ? "text-blue-500" : "text-current opacity-20"}`}
              >
          ...
          {allCategories.map((cat) => (
            ...
            >
              ...
              <div className="relative flex justify-center w-full">
                <div
                  className={`w-[1px] h-8 md:h-12 transition-all duration-700 ${isDarkMode ? "bg-white/10 group-hover:bg-white group-hover:h-24" : "bg-black/10 group-hover:bg-black group-hover:h-24"}`}
                />
                <div
                  className={`absolute top-0 w-1 h-1 rounded-full opacity-0 group-hover:opacity-100 group-hover:top-full transition-all duration-1000 ${isDarkMode ? "bg-white shadow-[0_0_10px_white]" : "bg-black shadow-[0_0_10px_black]"}`}
                />
              </div>
              <span
                className={`text-[9px] md:text-[12px] tracking-[0.3em] md:tracking-[0.4em] uppercase font-black transition-all text-center whitespace-nowrap ${isDarkMode ? "opacity-40 group-hover:opacity-100" : "opacity-40 group-hover:opacity-100"}`}
              >
                {cat}
              </span>
            </div>
...
const CategoryView: React.FC<{
  ...
  isDarkMode: boolean;
  CATEGORY_JP: Record<string, string>;
}> = ({ category, resources, onResourceClick, isDarkMode, CATEGORY_JP }) => {
  return (
    <div className="px-6 md:px-20 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      <div
        className={`mb-16 md:mb-32 flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-10 border-b pb-12 md:pb-20 ${isDarkMode ? "border-white/5" : "border-black/5"}`}
      >
        <div className="flex flex-col md:flex-row items-baseline gap-4 md:gap-10">
          <h2 className="text-4xl md:text-8xl lg:tex t-[11rem] font-black tracking-tighter uppercase opacity-90 leading-none">
            {category}
          </h2>
          <div className="vertical-text text-[10px] md:text-[12px] tracking-[0.8em] md:tracking-[1.2em] opacity-30 uppercase font-black whitespace-nowrap">
      ...
      {resources.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-32 md:py-56 text-center animate-in fade-in slide-in-from-bottom-12 duration-1000">
          <div
            className={`text-6xl md:text-[14rem] font-black opacity-[0.03] mb-8 select-none transition-colors ${isDarkMode ? "text-white" : "text-black"}`}
          >
            近日公開
          </div>
          <h3
            className={`text-4xl md:text-8xl font-black tracking-[0.3em] md:tracking-[0.4em] uppercase opacity-20 mb-8 transition-colors ${isDarkMode ? "text-white" : "text-black"}`}
          >
            COMING SOON
          </h3>
          <p
            className={`text-[9px] md:text-[11px] tracking-[0.6em] md:tracking-[1em] uppercase opacity-10 font-black transition-colors ${isDarkMode ? "text-white" : "text-black"}`}
          >
