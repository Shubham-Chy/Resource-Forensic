  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        ...
        />
        <div
          className={`absolute inset-0 opacity-70 group-hover:opacity-30 transition-opacity duration-700 ${isDarkMode ? "bg-black" : "bg-zinc-900"}`}
        />
      </div>

      <div
        className={`absolute top-6 right-8 vertical-text opacity-10 group-hover:opacity-60 transition-opacity duration-700 text-[9px] tracking-[1.2em] uppercase font-black text-white pointer-events-none`}
      >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center p-8 md:p-12 text-center">
        <div className="flex flex-col items-center gap-4 w-full max-w-sm">
          ...
          <div
            className={`h-[1px] bg-white/40 transition-all duration-1000 ${resource.isUpcoming ? "w-24 opacity-20" : "w-8 group-hover:w-32 group-hover:opacity-80"}`}
          />
          ...
          {resource.isUpcoming && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-20 pointer-events-none w-full">
              <span className="text-[40px] md:text-[50px] font-black opacity-[0.05] group-hover:opacity-[0.15] transition-opacity uppercase tracking-tighter mb-[-25px]">
                UPCOMING
              </span>
              <div className="flex flex-col items-center gap-2 mt-8">
                <span className="text-[9px] font-black tracking-[0.6em] text-blue-400 opacity-60 uppercase mb-1">
                  近日公開
                </span>
          ...
          <h3
            className={`text-xl md:text-2xl font-black tracking-tight leading-tight uppercase text-white break-words w-full drop-shadow-[0_4px_12px_rgba(0,0,0,0.5)] transition-all duration-500 ${resource.isUpcoming ? "opacity-30 group-hover:opacity-60" : "group-hover:scale-[1.02]"}`}
          >
            {resource.name}
          </h3>
        </div>
